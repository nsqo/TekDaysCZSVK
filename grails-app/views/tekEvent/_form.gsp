...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'organizer', 'error')} required">
	...
	<g:select id="organizer" name="organizer.id" from="${tekdaystwo.TekUser.list()}" optionKey="id" required="" value="${tekEventInstance?.organizer?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'startDate', 'error')} required">
	...
	<g:datePicker name="startDate" precision="minute" value="${tekEventInstance?.startDate}"  />
</div>
...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'endDate', 'error')} required">
	...
	<g:datePicker name="endDate" precision="minute"  value="${tekEventInstance?.endDate}"  />
</div>
...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'volunteers', 'error')} ">
	...
	<g:select name="volunteers" from="${tekdaystwo.TekUser.list()}" multiple="multiple" optionKey="id" size="5" value="${tekEventInstance?.volunteers*.id}" class="many-to-many"/>
</div>
...
<g:each in="${tekEventInstance?.tasks?}" var="t">
    <li><g:link controller="task" action="show" id="${t.id}">${t.title?.encodeAsHTML()}${t.completed?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
...
<g:each in="${tekEventInstance?.messages?}" var="m">
    <li><g:link controller="message" action="show" id="${m.id}">${m?.subject?.encodeAsHTML()} - ${m?.author?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
