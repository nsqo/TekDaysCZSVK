<%@ page import="tekdaystwo.Sponsor" %>
...
<div class="fieldcontain ${hasErrors(bean: sponsorInstance, field: 'events', 'error')} ">
	...
	<g:select name="events" from="${tekdaystwo.TekEvent.list()}" multiple="multiple" optionKey="id" size="5" value="${sponsorInstance?.events*.id}" class="many-to-many"/>
</div>
